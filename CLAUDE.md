...
- **Bottom sheets et navbar** — les sheets sont poussées sur le Navigator du `ShellRoute`, alors que la navbar est le `bottomNavigationBar` du Scaffold d'`AppShell` : la barre **peint donc par-dessus** les sheets et masquait leur bas. `bottomInsetFor(context)` (`core/widgets/bottom_sheet_widget.dart`) donne le padding bas : `AppSpacing.bottomBarInset` (124 = 24 + 68 + 32, hauteur réelle de la barre) clavier fermé, `viewInsets.bottom + AppSpacing.lg` clavier ouvert — les deux ne s'additionnent jamais, le clavier recouvrant déjà la barre. Utilisé par `BottomSheetWidget` et par la sheet article (`HomeArticleCardWidget`, contenu custom qui ne passe pas par la classe partagée). Tous les appelants de `BottomSheetWidget` vivent sous le shell ; `create-pet` (hors shell, donc sans barre) n'en utilise pas.
