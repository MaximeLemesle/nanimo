...
- **Strip d'animaux** (`HomePetListWidget`) : rangée horizontale scrollable d'avatars (`PetAvatarWidget` medium) + prénom sous chacun. Tap → sélection du pet (`PetDetailsCubit` partagé) + push pet page. Widget **stateful** (`ScrollController`) : un `ShaderMask` fond le bord droit tant qu'il reste des animaux hors écran, et disparaît en fin de course — sans ce fondu, rien ne laissait deviner que la rangée slide. Aucun spacer réservé sous le fondu : il s'éteint déjà à l'arrivée, et un spacer conditionnel ferait sauter le layout en changeant le `maxScrollExtent`.
